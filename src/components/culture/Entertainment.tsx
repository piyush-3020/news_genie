...
const Entertainment = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const result = await info("entertainment");
      setData(result);
    };
    fetchData();
  }, []);
  ...
  return (
    <>
      {data?.data?.articles?.map((val: {
        ...
        // Ensure that the article has a valid title, description, and urlToImage
        ...
        return null; // Ensures that invalid articles are skipped
      })}
    </>
  );
};
