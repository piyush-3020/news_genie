...
const Technology = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await sport("technology");
        setData(result); // Store the fetched data in the state
      } catch (error) {
        setError("Failed to load technology news."); // Set error message on failure
      } finally {
        ...
        if (val.title !== "[Removed]" && val.description !== "[Removed]" && val.urlToImage !== null) {
          return (
            <div className="news-card" key={val.url}>
              <div className="par1">
                <img
                  src={val.urlToImage || "https://via.placeholder.com/150"} // Generic technology fallback image
                  alt="News"
