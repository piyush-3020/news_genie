...
const Health = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const result = await info("health");
      setData(result);
    };
    fetchData();
  ...
  return (
    <>
      {data?.data?.articles?.map((val: {
        ...
        // Ensure the article has valid title, description, and urlToImage
        if (val.title !== "[Removed]" && val.description !== "[Removed]" && val.urlToImage !== null) {
          return (
            <div className="news-card" key={val.url}>
              <div className="par1">
                <img
                  src={val.urlToImage || "https://blogs.dpuerp.in/images/blog/10/524-prevention-is-better-than-cure.jpg"}
                  alt="News"
